...
var net = require("net"),
    ...
    connection_id = 0,
    //default_port = 6379,
    default_port = 6380, //ledis' default port is 6380 --wenyekui<wyk4true@gmail.com>
...
exports.LedisClient = RedisClient; //export RedisClient as LedisClient --wenyekui<wyk4true@gmail.com>
...
RedisClient.prototype.on_error = function (msg) {
    var message = "Ledis connection to " + this.host + ":" + this.port + " failed - " + msg;
    ...
    self.send_command("auth", [this.auth_pass], function (err, res) {
        if (err) {
            if (err.toString().match("LOADING")) {
                ...
                console.log("Ledis still loading, trying to authenticate later");
                setTimeout(function () {
                    self.do_auth();
        ...
        this.on_ready(); //ready_check uses `info` command, ledisDB doesn't have `info` command, so we directly `on_ready` without `ready_check`  --wenyekui<wyk4true@gmail.com>
    });
...
RedisClient.prototype.init_parser = function () {
    ...
    this.reply_parser.on("error", function (err) {
        self.emit("error", new Error("Ledis reply parser error: " + err.stack));
    });
};
...
RedisClient.prototype.on_info_cmd = function (err, res) {
    ...
    if (!obj.loading || (obj.loading && obj.loading === "0")) {
        if (exports.debug_mode) {
            console.log("Ledis server ready.");
        }
        this.on_ready();
    } else {
        ...
        if (exports.debug_mode) {
            console.log("Ledis server still loading, trying again in " + retry_time);
        }
        setTimeout(function () {
            self.ready_check();
        }, retry_time);
...
RedisClient.prototype.connection_gone = function (why) {
    ...
    if (exports.debug_mode) {
        console.warn("Ledis connection is gone from " + why + " event.");
    }
    this.connected = false;
    this.ready = false;
    ...
    this.flush_and_error("Ledis connection gone from " + why + " event.");
    ...
    if (this.max_attempts && this.attempts >= this.max_attempts) {
        this.retry_timer = null;
        // TODO - some people need a "Ledis is Broken mode" for future commands that errors immediately, and others
...
//exports.Multi = Multi; //do not export Multi, 'cause ledis does not support multi  --wenyekui<wyk4true@gmail.com>
